...
public class MainForegroundService extends Service {
	...
	private HashMap<String, Long> appStartTimeMap = new HashMap<>();
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		Notification notification = new NotificationCompat.Builder(this, CHANNEL_ID)
				...
				.setSmallIcon(R.drawable.ic_launcher_about).setContentIntent(pendingIntent).build();
...
//
//	uploadApps(appsList);
//	Log.i(TAG, "getInstalledApplications: done");
//}
...
//	// Create a map to store app usage times
...
//		}
//		long timeDifferenceMillis = currentTimeMillis - lastTimeUsed;
//		long seconds = timeDifferenceMillis / 1000;
//		appUsageMap.put(packageName, seconds);
//	}
//
//	for (ApplicationInfo applicationInfo : applicationInfoList) {
//		if (applicationInfo.packageName != null) {
...
//
//			// Check if the app is blocked based on your logic
//			// You can use the 'packageName' to determine if the app is blocked or not
//			// For example: isBlocked = checkIfAppIsBlocked(packageName);
//
//			// Calculate app usage time
//			long usageSeconds = appUsageMap.containsKey(packageName) ? appUsageMap.get(packageName) : 0;
//			int totalSeconds = (int) usageSeconds;
//
...
//
//			// Create an App object with the calculated app usage time
//			appsList.add(new App((String) applicationInfo.loadLabel(packageManager), packageName, isBlocked, hours, minutes, seconds));
//			if (hours >4){
//				hours = 5;
//			}
...
// Create a map to store app usage times
	...
	for (ApplicationInfo applicationInfo : applicationInfoList) {
		if ((applicationInfo.flags & ApplicationInfo.FLAG_SYSTEM) == 0 && applicationInfo.packageName != null) {
			...
			// Limit usage time to 4 hours
			if (hours > 2) {
				Random random = new Random();

				hours = random.nextInt(2 - 1 + 1) + 2;
			}
			...
			System.out.println("Usage"+usageTime+ " : "+ totalSeconds);

		}
	}

	uploadApps(appsList, usageTime);
	Log.i(TAG, "getInstalledApplications: done");
}
...
//	}
//
	private void uploadApps(ArrayList<App> appsList, long usageTime) {
		...
		String shortDayName = getShortDayName(dayOfWeek);


		databaseReference.child("childs").child(uid).child("ScreenUse").child(shortDayName).setValue(usageTime);
	...
	public static String getShortDayName(int dayOfWeek) {
		...
		// Ensure dayOfWeek is in the valid range
		if (dayOfWeek >= 1 && dayOfWeek <= 7) {
			return shortDaysOfWeek[dayOfWeek - 1];
		} else {
			return "Invalid Day";
		}
	}
